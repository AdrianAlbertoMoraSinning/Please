(() => {
  ...
  function openAssignment(id){
    const a=data.assignments.find(x=>x.id===id);if(!a)return;const p=data.providers.find(x=>x.id===a.provider_id),j=a.jobs||{},s=localParts(a.scheduled_start),e=localParts(a.scheduled_end),c=j.customers||{},items=billingForJob(j.id).filter(x=>x.assignment_id===a.id||(!x.assignment_id&&x.provider_id===a.provider_id)),change=scheduleChangeFor(a.id),hours=Math.max(.25,Math.round(((new Date(a.scheduled_end)-new Date(a.scheduled_start))/3600000)*4)/4);
