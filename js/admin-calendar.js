(() => {
  ...
  let data={providers:[],provider_services:[],services:[],availability:[],exceptions:[],assignments:[],needs_assignment:[],reassignment_assignments:[],source_requests:[],provider_rates:[],job_billing_items:[],final_invoices:[],invoice_items:[],schedule_changes:[]};
  let billingItems=[];
  let teamAssignments=[];
  ...
  function finalInvoiceForJob(jobId){return (data.final_invoices||[]).filter(x=>x.job_id===jobId).sort((a,b)=>String(b.created_at||'').localeCompare(String(a.created_at||'')))[0]||null;}
  function invoiceItemsFor(invoiceId){return (data.invoice_items||[]).filter(x=>x.invoice_id===invoiceId).sort((a,b)=>(a.sort_order||0)-(b.sort_order||0));}
  ...
  async function submitJob(e){
    ...
    if(!teamAssignments.length)return showAlert('Add at least one Provider.');
    const ids=new Set();
    for(let i=0;i<teamAssignments.length;i++){
      const a=teamAssignments[i];
      if(!a.provider_id||!a.date||!a.start||!a.end)return showAlert(`Provider ${i+1}: provider, date, start and end are required.`);
      if(ids.has(a.provider_id))return showAlert('The same Provider cannot be added twice to the same Job.');ids.add(a.provider_id);
      if(!a.billing_items?.length)return showAlert(`Provider ${i+1}: add at least one billing item.`);
      if(!['00','15','30','45'].includes(a.start.slice(3,5))||!['00','15','30','45'].includes(a.end.slice(3,5)))return showAlert('Provider schedules must use 15-minute increments.');
    }
    if(!confirmFinancialIntegrity(teamAssignments))return;
    const financialIssues=financialIntegrityIssues(teamAssignments);
    const assignments=teamAssignments.map((a,i)=>({provider_id:a.provider_id,scheduled_start:localToIso(a.date,a.start),scheduled_end:localToIso(a.date,a.end),assignment_message:a.assignment_message||$('job-message').value.trim()||'',is_primary:Boolean(a.is_primary),sequence_no:Number(a.sequence_no)||i+1,replacement:Boolean(a.replacement),billing_items:a.billing_items.map(x=>({provider_service_rate_id:x.provider_service_rate_id,quantity:Number(x.quantity),customer_unit_rate:Number(x.customer_unit_rate??x.unit_rate),provider_compensation_method:x.provider_compensation_method,provider_compensation_value:x.provider_compensation_value}))}));
    const payload={job_id:existing,replace_assignment_id:reassignmentTargetAssignment?.id||null,service_id:serviceId,assignments,allow_nonpositive_margin:financialIssues.length>0};
    submitBtn.disabled=true;submitBtn.textContent='SAVING & SENDING ASSIGNMENTS…';
    try{
      const result=await api('/.netlify/functions/admin-job-action',{method:'POST',body:JSON.stringify({action:'REASSIGN_MULTI_WITH_BILLING',payload})});
      closeDrawer();const first=teamAssignments[0];weekStart=startOfWeek(new Date(`${first.date}T12:00:00`));
      const success=`${result.job_reference||'Job'} corrected and reassigned with ${result.provider_count||teamAssignments.length} new Provider assignment${(result.provider_count||teamAssignments.length)===1?'':'s'}.${result.added_provider_count?` ${result.added_provider_count} additional Provider${result.added_provider_count===1?' was':'s were'} added to the same Job.`:''}${result.billing_items_replaced!=null?` ${result.billing_items_replaced} prior billing item${result.billing_items_replaced===1?'':'s'} replaced.`:''}${result.provider_rates_updated?` ${result.provider_rates_updated} Provider Rate${result.provider_rates_updated===1?' was':'s were'} saved to the corresponding Provider profile${result.provider_rates_updated===1?'':'s'}.`:''}`;
  ...
  async function saveAdminScheduleEdit(a,j){
    const date=$('admin-edit-service-date')?.value,start=$('admin-edit-service-start')?.value,hours=Number($('admin-edit-service-hours')?.value),applyTeam=$('admin-edit-service-team')?.checked!==false,syncBilling=$('admin-edit-service-billing')?.checked!==false;
    if(!date||!start)return showAlert('Service Date and Start Time are required.');
    if(!/^\d{2}:(00|15|30|45)$/.test(start))return showAlert('Start Time must use :00, :15, :30 or :45.');
    if(!Number.isFinite(hours)||hours<.25||hours>72||Math.abs(hours*4-Math.round(hours*4))>1e-8)return showAlert('Service Hours must use 0.25-hour increments between 0.25 and 72.');
    const scheduledStart=localToIso(date,start),scheduledEnd=new Date(new Date(scheduledStart).getTime()+Math.round(hours*60)*60000).toISOString(),btn=$('admin-save-service-schedule');
    if(!confirm(`${applyTeam?'Update the active service team':'Update this Provider only'} to ${date} ${time12(start)} for ${hours.toFixed(2)} hour${hours===1?'':'s'}?${syncBilling?' Hourly billing quantities will also be synchronized.':''}`))return;
    btn.disabled=true;btn.textContent='SAVING…';
    try{
      const result=await api('/.netlify/functions/admin-active-job-update',{method:'POST',body:JSON.stringify({job_id:j.id,assignment_id:a.id,apply_to_team:applyTeam,scheduled_start:scheduledStart,scheduled_end:scheduledEnd,estimated_duration_minutes:Math.round(hours*60),sync_hourly_billing:syncBilling,sync_source_request:true,reason:'Schedule / hours changed from Master Calendar'})});
      $('assignment-modal').hidden=true;weekStart=startOfWeek(new Date(`${date}T12:00:00`));await loadCalendar();showAlert(`${result.job_reference||j.reference} updated: ${result.updated_assignments} Provider schedule${result.updated_assignments===1?'':'s'}${result.hourly_billing_items_updated?` and ${result.hourly_billing_items_updated} hourly billing item${result.hourly_billing_items_updated===1?'':'s'}`:''}.`,'success');
    }catch(e){showAlert(e.message);}finally{if(btn){btn.disabled=false;btn.textContent='SAVE SCHEDULE & HOURS →';}}
  }

  function openAssignment(id){
    const a=data.assignments.find(x=>x.id===id);if(!a)return;const p=data.providers.find(x=>x.id===a.provider_id),j=a.jobs||{},s=localParts(a.scheduled_start),e=localParts(a.scheduled_end),c=j.customers||{},jobItems=billingForJob(j.id),items=jobItems.filter(x=>x.assignment_id===a.id||(!x.assignment_id&&x.provider_id===a.provider_id)),finalInvoice=finalInvoiceForJob(j.id),finalItems=finalInvoice?invoiceItemsFor(finalInvoice.id):[],change=scheduleChangeFor(a.id),hours=Math.max(.25,Math.round(((new Date(a.scheduled_end)-new Date(a.scheduled_start))/3600000)*4)/4);
    const finalBillHtml=finalInvoice?`<h3>Final Customer Billing · ${esc(finalInvoice.invoice_number)}</h3><div class="calendar-modal-billing">${finalItems.map(x=>`<div><span>${esc(x.description)} · ${Number(x.qty).toFixed(2)} ${esc(x.unit)}</span><strong>${money(x.line_total)}</strong><small>Final invoice customer value</small></div>`).join('')}<div class="grand"><span>Subtotal</span><strong>${money(finalInvoice.subtotal)}</strong></div><div><span>GST</span><strong>${money(finalInvoice.gst_amount)}</strong></div><div class="grand"><span>Total · ${esc(finalInvoice.status)}</span><strong>${money(finalInvoice.total_amount)}</strong></div></div>`:'';
    const snapshotBillHtml=items.length?`<h3>${finalInvoice?'Provider / Job Billing Snapshot':'Customer Billing'}</h3><div class="calendar-modal-billing">${items.map(x=>`<div><span>${esc(x.service_name||'Service')} · ${esc(x.description)} · ${Number(x.quantity).toFixed(2)} ${esc(x.unit)}</span><strong>${money(x.customer_line_total??x.line_total)}</strong><small>Provider ${x.provider_line_total==null?'—':money(x.provider_line_total)} · Profit ${x.gross_profit==null?'—':money(x.gross_profit)}</small></div>`).join('')}<div class="grand"><span>${finalInvoice?'Job subtotal':'Subtotal'}</span><strong>${money(items.reduce((n,x)=>n+Number((x.customer_line_total??x.line_total)??0),0))}</strong></div></div>`:'';
    const billHtml=finalBillHtml+snapshotBillHtml;
    ...
    const adminEdit=['PENDING','CONFIRMED'].includes(a.status)?`<div class="admin-detail-section"><div class="calendar-modal-actions"><button id="open-admin-schedule-edit" class="admin-outline-button" type="button">EDIT SERVICE SCHEDULE & HOURS</button></div><div id="admin-schedule-editor" class="job-internal-note" hidden><strong>Administration schedule correction</strong><p>Use this when the customer changes the date, start time or number of service hours after the Job was created.</p><div class="developer-form-grid"><label>Service Date<input id="admin-edit-service-date" type="date" value="${esc(s.date)}"></label><label>Start Time<input id="admin-edit-service-start" type="time" step="900" value="${esc(s.time)}"></label><label>Service Hours<input id="admin-edit-service-hours" type="number" min="0.25" max="72" step="0.25" value="${hours.toFixed(2)}"></label></div><label class="developer-check-row"><input id="admin-edit-service-team" type="checkbox" checked> Apply the date/time/hours to all active Providers on this Job</label><label class="developer-check-row"><input id="admin-edit-service-billing" type="checkbox" checked> Adjust hourly billing quantities to match the new service hours</label><div class="calendar-modal-actions"><button id="admin-save-service-schedule" class="btn primary" type="button">SAVE SCHEDULE & HOURS →</button><button id="admin-cancel-service-schedule" class="admin-outline-button" type="button">CANCEL</button></div></div></div>`:'';
    $('assignment-modal-content').innerHTML=`<span class="status-badge status-${a.status.toLowerCase()}">${esc(statusLabel(a.status))}</span><h2>${esc(j.reference||'Assignment')}</h2><div class="admin-detail-grid"><div><span>Provider</span><strong>${esc(p?.display_name||'')}</strong></div><div><span>Service</span><strong>${esc(j.service_name||'')}</strong></div><div><span>Date</span><strong>${esc(s.date)}</strong></div><div><span>Time</span><strong>${time12(s.time)}–${time12(e.time)}</strong></div><div><span>Customer</span><strong>${esc([c.first_name,c.last_name].filter(Boolean).join(' ')||'—')}</strong></div><div><span>Customer Contact</span><strong>${esc(c.email||c.phone||'—')}</strong></div></div>${billHtml}<h3>Work Address</h3><p>${esc(j.work_address||'')}</p><h3>Work Description</h3><p class="admin-prewrap">${esc(j.work_description||'')}</p>${a.assignment_message?`<h3>Message to Provider</h3><p>${esc(a.assignment_message)}</p>`:''}${a.provider_response_note?`<h3>Provider Response</h3><p>${esc(a.provider_response_note)}</p>`:''}${changeHtml}${adminEdit}<div class="calendar-modal-actions">${['PENDING','CONFIRMED'].includes(a.status)?'<button id="cancel-assignment" class="admin-danger-button" type="button">CANCEL ASSIGNMENT</button>':''}</div>`;
    ...
    const openEdit=$('open-admin-schedule-edit'),editor=$('admin-schedule-editor');if(openEdit&&editor)openEdit.onclick=()=>{editor.hidden=false;openEdit.hidden=true;};const cancelEdit=$('admin-cancel-service-schedule');if(cancelEdit)cancelEdit.onclick=()=>{editor.hidden=true;if(openEdit)openEdit.hidden=false;};const saveEdit=$('admin-save-service-schedule');if(saveEdit)saveEdit.onclick=()=>saveAdminScheduleEdit(a,j);
  ...
  function bindEvents(){
    ...
    on('job-add-provider','click',()=>{const base=teamAssignments[0]||teamDefault();if(reassignmentMode){const next=Math.max(0,...teamAssignments.map((a,i)=>Number(a.sequence_no)||i+1))+1;teamAssignments.push({...teamDefault(base.date,base.start,base.end),is_primary:false,sequence_no:next,replacement:false});}else teamAssignments.push(teamDefault(base.date,base.start,base.end));renderTeam();});
