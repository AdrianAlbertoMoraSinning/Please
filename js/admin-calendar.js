(() => {
  ...
  function rateLabel(r){const sn=data.services.find(s=>s.id===r.service_id)?.name||'Service';return `${sn} — ${r.rate_name} — ${money(r.customer_rate)} / ${String(r.billing_unit||'service').replace('_',' ')}`;}
  ...
  function renderBillingPicker(){const pid=$('job-provider')?.value,picker=$('job-billing-rate-picker');if(!picker)return;const rates=providerRates(pid);picker.innerHTML='<option value="">Select rate item</option>'+rates.map(r=>`<option value="${r.id}">${esc(rateLabel(r))}</option>`).join('');if(!pid)picker.innerHTML='<option value="">Select provider first</option>';else if(!rates.length)picker.innerHTML='<option value="">Provider has no active Service Rates</option>';renderBillingItems();}
  function renderBillingItems(){const box=$('job-billing-items'),empty=$('job-billing-empty'),readonly=!!$('job-existing-id')?.value;if(!box)return;empty.hidden=billingItems.length>0;box.innerHTML=billingItems.map((x,i)=>`<div class="calendar-billing-row" data-index="${i}"><div class="calendar-billing-name"><strong>${esc(x.service_name||'Service')} · ${esc(x.description)}</strong><small>${esc(String(x.unit||'service').replace('_',' '))}</small></div><label>Qty<input class="billing-qty" data-index="${i}" type="number" min="0.01" step="0.01" value="${Number(x.quantity).toFixed(2)}" ${readonly?'disabled':''}></label><label>Rate<input class="billing-rate" data-index="${i}" type="number" min="0" step="0.01" value="${Number(x.unit_rate).toFixed(2)}" ${readonly?'disabled':''}></label><strong class="billing-line-total">${money(Number(x.quantity)*Number(x.unit_rate))}</strong>${readonly?'':`<button type="button" class="admin-danger-button billing-remove" data-index="${i}">×</button>`}</div>`).join('');box.querySelectorAll('.billing-qty,.billing-rate').forEach(el=>el.addEventListener('input',()=>{const i=Number(el.dataset.index);if(el.classList.contains('billing-qty'))billingItems[i].quantity=Math.max(.01,Number(el.value)||0);else billingItems[i].unit_rate=Math.max(0,Number(el.value)||0);renderBillingTotals();const row=el.closest('.calendar-billing-row');if(row)row.querySelector('.billing-line-total').textContent=money(billingItems[i].quantity*billingItems[i].unit_rate);}));box.querySelectorAll('.billing-remove').forEach(b=>b.onclick=()=>{billingItems.splice(Number(b.dataset.index),1);renderBillingItems();});renderBillingTotals();}
  function renderBillingTotals(){const subtotal=billingItems.reduce((n,x)=>n+(Number(x.quantity)||0)*(Number(x.unit_rate)||0),0),gst=subtotal*.05;if($('job-billing-subtotal'))$('job-billing-subtotal').textContent=money(subtotal);if($('job-billing-gst'))$('job-billing-gst').textContent=money(gst);if($('job-billing-total'))$('job-billing-total').textContent=money(subtotal+gst);}
  function addBillingItem(){const id=$('job-billing-rate-picker')?.value,r=providerRates($('job-provider')?.value).find(x=>x.id===id);if(!r)return showAlert('Select an active Provider Service Rate first.');billingItems.push({provider_service_rate_id:r.id,service_id:r.service_id,service_name:data.services.find(s=>s.id===r.service_id)?.name||'Service',description:r.rate_name,quantity:defaultQtyForRate(r),unit:r.billing_unit,unit_rate:Number(r.customer_rate)||0});renderBillingItems();}
  ...
  async function submitJob(e){
    ...
    if(!existing){if(!billingItems.length)return showAlert('Add at least one Customer Billing item from this Provider’s active Service Rates.');payload.billing_items=billingItems.map(x=>({provider_service_rate_id:x.provider_service_rate_id,quantity:Number(x.quantity),unit_rate:Number(x.unit_rate)}));}
