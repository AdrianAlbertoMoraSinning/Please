(()=>{
 ...
 form.addEventListener('submit',async e=>{e.preventDefault();alertBox.hidden=true;if(!service.value)return show('Please select a service.');const hours=Number(form.elements.estimated_hours?.value),time=String(form.elements.preferred_start_time?.value||'');if(!Number.isFinite(hours)||hours<0.25||hours>72)return show('Please enter the estimated number of hours.');if(!/^\d{2}:(00|15|30|45)$/.test(time))return show('Please select a start time in 15-minute increments.');if(isMoving()){const b=Number(form.elements.moving_bedrooms.value),sf=Number(form.elements.moving_square_feet.value),inv=form.elements.moving_inventory.value.trim();if(!Number.isFinite(b)||b<0||!Number.isFinite(sf)||sf<=0||!inv)return show('Please complete all Moving details.');}
