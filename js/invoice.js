(async()=>{
 ...
 const publicNote=v=>{const t=String(v||'').trim();if(!t)return'';if(/^Automatically prepared from completed Job\b/i.test(t))return'';return t;};
 function banner(i){
   ...
   if(payment==='success'||sessionId)return `<div class="invoice-result-banner info"><strong>Payment submitted.</strong><span>Payment confirmation is checked automatically. Paid invoices remain visible from this link.</span></div>`;
   if(i.payment_status==='PENDING')return `<div class="invoice-result-banner info"><strong>Payment in progress.</strong><span>A secure online payment was started for this invoice. If you did not finish, you can safely start again.</span></div>`;
   return '';
 }
 function paymentPanel(i,p,balance){
   const cur=i.currency||'CAD';
   if(i.is_paid||i.payment_status==='PAID')return `<div class="invoice-payment-panel paid-panel"><h3>Payment received</h3><p>This invoice has been paid. Thank you.</p>${i.paid_at?`<p><strong>Paid on:</strong> ${esc(date(i.paid_at))}</p>`:''}${i.payment_reference?`<p><strong>Payment confirmation:</strong> ${esc(i.payment_reference)}</p>`:''}</div>`;
   if(p.stripe_enabled)return `<div class="invoice-payment-panel"><h3>Pay securely online</h3><p>Click below to securely pay your invoice online.</p><button id="pay-now" class="btn primary" type="button">${payment==='cancelled'?'PAY AGAIN':'PAY'} ${money(balance,cur)} →</button><div id="pay-error" class="form-alert" hidden></div></div>`;
