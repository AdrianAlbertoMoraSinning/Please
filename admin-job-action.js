...
async function notifyCustomerJob(jobId,kind='SCHEDULED'){
  try{
    ...
    let intro=kind==='CANCELLED'?`Hi ${c.first_name||'there'}, this PLEASE service has been cancelled or returned for reassignment.`:`Hi ${c.first_name||'there'}, PLEASE has created the service Job and sent the assignment to the selected professional(s) for confirmation.`;
    return notify.send({to:c.email,subject:`PLEASE — ${title} (${j.reference})`,title,intro,details:[['Job',j.reference],['Service',j.service_name],['Status',j.status],['Address',j.work_address]],message:kind==='CANCELLED'?'PLEASE Administration will contact you if a replacement schedule or provider is needed.':'The schedule is not final until PLEASE confirms the provider response.',ctaLabel:'Track Your Request',ctaUrl:`${notify.baseUrl()}/track-request.html`,idempotencyKey:`please-job-${jobId}-${kind}`});
