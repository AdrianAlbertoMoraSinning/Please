...
## STEP 19.5 — Service Lifecycle Editing, Follow-up Services & Portal Manuals

Administration now exposes a lifecycle-safe editing model: unassigned Requests remain editable, active Jobs keep schedule/service/rate control, PENDING/CONFIRMED Providers can be replaced from Jobs without creating a new Job, returning customers can start a new follow-up service with their customer/address data prefilled, completed Jobs route final commercial review to the Invoice instead of rewriting execution history, and both Administrator and Provider manuals are available inside the portals. Service Type changes on active Jobs validate Provider authorization before save. No new SQL migration is required. See `STEP19_5_SERVICE_LIFECYCLE_EDITING_MANUALS.md`.
...
Production repair for CAL inside PLEASE:

- Includes the CAL Netlify backend functions in the deploy hotfix so `Accept Agreement & Continue` can call `/.netlify/functions/cal-legal-acceptance` successfully.
- Prevents raw Netlify `Page not found` HTML from appearing in browser alerts if a backend endpoint is missing.
- Ensures the `CAL Accounting` sidebar link appears consistently across all root PLEASE Administration operation pages, including Service Maintenance.

No new SQL is introduced by STEP 16.0.2. Use the existing `supabase/STEP16_0_CAL_PLEASE_ACCOUNTING_BRIDGE.sql` if STEP 16.0 has not yet been run.

## STEP 16.0 — CAL 1.5 Accounting Bridge

This release embeds CAL 1.5 as an independent accounting portal under `/cal/` and adds server-side accounting automation for PLEASE financial events.
...
- `STEP16_0_CAL_PLEASE_ACCOUNTING_BRIDGE.md`

The bridge posts idempotent journal entries for issued invoices, Stripe/manual payments, invoice voids, provider payables and provider payment completion. Operational actions continue even if accounting sync fails; failures are recorded in the accounting outbox for retry.
