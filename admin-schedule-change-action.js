...
exports.handler=async event=>{
  ...
  try{
    ...
    if(action==='REJECT'){
      ...
      const j=await notify.jobContext(req.job_id).catch(()=>null),notices=[];
      notices.push(await notify.sendProvider(req.provider_id,{subject:`PLEASE — Schedule Change Not Approved (${j?.reference||'Job'})`,title:'Schedule change not approved',intro:'PLEASE Administration reviewed your schedule request. The original assignment schedule remains in effect.',details:[['Job',j?.reference],['Current schedule',`${notify.formatDateTime(req.current_start)} → ${notify.formatDateTime(req.current_end)}`],['Requested schedule',`${notify.formatDateTime(req.proposed_start)} → ${notify.formatDateTime(req.proposed_end)}`]],message:note,ctaLabel:'Open Provider Portal',ctaUrl:`${notify.baseUrl()}/provider.html#assignments`,idempotencyKey:`please-provider-schedule-rejected-${id}`}));
      if(j?.customers?.email)notices.push(await notify.send({to:j.customers.email,subject:`PLEASE — Schedule Update (${j.reference})`,title:'Your original PLEASE schedule remains',intro:`Hi ${j.customers.first_name||'there'}, a provider schedule-change request was reviewed and not applied. Your current service schedule remains unchanged.`,details:[['Job',j.reference],['Service',j.service_name],['Current schedule',`${notify.formatDateTime(req.current_start)} → ${notify.formatDateTime(req.current_end)}`]],ctaLabel:'Track Your Request',ctaUrl:`${notify.baseUrl()}/track-request.html`,idempotencyKey:`please-customer-schedule-rejected-${id}`}));
      return lib.json(200,{ok:true,status:'REJECTED',updated_assignments:0,notifications_sent:notices.filter(x=>x?.sent).length});
    }
    ...
    const j=await notify.jobContext(req.job_id).catch(()=>null),notices=[];
    for(const a of targets){notices.push(await notify.sendProvider(a.provider_id,{subject:`PLEASE — Schedule Change Approved (${j?.reference||'Job'})`,title:'PLEASE approved the new service schedule',intro:'Your PLEASE assignment calendar has been updated.',details:[['Job',j?.reference],['Service',j?.service_name],['New schedule',`${notify.formatDateTime(req.proposed_start)} → ${notify.formatDateTime(req.proposed_end)}`]],message:note,ctaLabel:'Open Provider Portal',ctaUrl:`${notify.baseUrl()}/provider.html#calendar`,idempotencyKey:`please-provider-schedule-approved-${id}-${a.id}`}));}
    if(j?.customers?.email)notices.push(await notify.send({to:j.customers.email,subject:`PLEASE — Service Schedule Updated (${j.reference})`,title:'Your PLEASE service schedule was updated',intro:`Hi ${j.customers.first_name||'there'}, PLEASE Administration approved a schedule update for your service.`,details:[['Job',j.reference],['Service',j.service_name],['New schedule',`${notify.formatDateTime(req.proposed_start)} → ${notify.formatDateTime(req.proposed_end)}`]],ctaLabel:'Track Your Request',ctaUrl:`${notify.baseUrl()}/track-request.html`,idempotencyKey:`please-customer-schedule-approved-${id}`}));
    return lib.json(200,{ok:true,status:'ACCEPTED',apply_to_team:applyTeam,updated_assignments:targets.length,notifications_sent:notices.filter(x=>x?.sent).length});
