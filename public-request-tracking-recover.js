...
exports.handler=async event=>{
  ...
  try{
    ...
    const n=await notify.send({to:req.email,subject:`PLEASE — Tracking Link (${req.reference})`,title:'Your PLEASE tracking link',intro:'A secure tracking link was requested for your service request.',details:[['Request',req.reference]],message:'Use the secure link below to view the current request status. If you did not request this link, you can ignore this email.',ctaLabel:'Track Your Request',ctaUrl:trackingUrl,idempotencyKey:`please-tracking-recover-${req.id}-${hash(token).slice(0,16)}`});
    return lib.json(200,{reference:req.reference,tracking_token:token,email_sent:!!n?.sent});
  }catch(e){
