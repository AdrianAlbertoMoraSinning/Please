...
async function getData(){
  ...
  const payees=(parties||[]).filter(p=>{const s=rolesByParty.get(p.id)||new Set();return s.has('EMPLOYEE')||s.has('CONTRACTOR')||s.has('OPERATIONAL_PROVIDER')});
  ...
  return{expenses:normalized,operationalExpenses:(legacyExpenses||[]).map(x=>({...x})),parties:parties||[],payees,accounts:(accounts||[]).filter(a=>['EXPENSE','ASSET'].includes(a.account_type)),taxCodes:taxCodes||[],financialAccounts:[...fm.values()],counts};
}
...
async function normalizeLines(rawLines,expenseDate){
  ...
  return rows.map((x,i)=>{
    ...
    if(['PREPAID','FIXED_ASSET','INVENTORY'].includes(classification)&&account.account_type!=='ASSET')bad(`Line ${i+1}: ${classification} classification requires an Asset account.`);
