  ...
  try{
    ...
    if(req.job_id){
      ...
      if(job){
        ...
        try{evidence=await lib.sbJson(`/rest/v1/job_service_evidence?select=id,assignment_id,provider_id,evidence_type,storage_path,created_at&job_id=eq.${encodeURIComponent(job.id)}&status=eq.COMMITTED&order=created_at.asc`);for(const x of evidence){x.url=await signed(x.storage_path);const t=team.find(a=>a.id===x.assignment_id)||team.find(a=>a.provider_id===x.provider_id);x.provider_name=t?.provider_name||null;}}catch(_){}
