  ...
  try{
    ...
    if(req.job_id){
      ...
      if(job){
        ...
        team=[];for(const a of assignments||[]){const pr=pmap.get(a.provider_id)||null;if(pr)pr.profile_photo_url=pr.profile_image_path?`/.netlify/functions/provider-profile-photo?provider_id=${encodeURIComponent(pr.id)}&tracking_token=${encodeURIComponent(token)}`:safePublicUrl(pr.profile_image_url);team.push({...a,provider_name:pr?.display_name||null,provider_title:pr?.public_title||null,provider_photo_url:pr?.profile_photo_url||null});}
