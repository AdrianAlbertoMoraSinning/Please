  ...
  try{
    ...
    if(req.job_id){
      ...
      if(job){
        ...
        try{evidence=await lib.sbJson(`/rest/v1/job_service_evidence?select=id,assignment_id,provider_id,evidence_type,storage_path,created_at&job_id=eq.${encodeURIComponent(job.id)}&status=eq.COMMITTED&evidence_type=in.(ARRIVAL,COMPLETION)&order=created_at.asc`);evidence=(evidence||[]).filter(x=>team.some(a=>a.id===x.assignment_id||a.provider_id===x.provider_id));for(const x of evidence){x.url=await signed(x.storage_path);const t=team.find(a=>a.id===x.assignment_id)||team.find(a=>a.provider_id===x.provider_id);x.provider_name=t?.provider_name||null;}}catch(_){}
        ...
        const evByAssignment=new Map();for(const e of serviceEvents||[]){if(!e.assignment_id)continue;if(!evByAssignment.has(e.assignment_id))evByAssignment.set(e.assignment_id,new Set());evByAssignment.get(e.assignment_id).add(e.event_type);}for(const t of team){const es=evByAssignment.get(t.id)||new Set();t.live_status=t.status==='COMPLETED'||es.has('COMPLETED')?'COMPLETED':es.has('STARTED')?'IN_PROGRESS':es.has('ARRIVED')?'ARRIVED':t.status;}assignment=team.find(a=>a.is_primary)||team[0]||null;provider=assignment?{display_name:assignment.provider_name,public_title:assignment.provider_title,profile_photo_url:assignment.provider_photo_url}:null;
