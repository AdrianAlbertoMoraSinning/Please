...
async function activeServices(){
  try{return await lib.sbJson('/rest/v1/services?select=id,name,short_description,active&active=eq.true&order=sort_order.asc,name.asc');}
    ...
    console.warn('admin-calendar-data:services-primary',primaryError?.message||primaryError);
    try{return await lib.sbJson('/rest/v1/services?select=id,name,active&active=eq.true&order=name.asc');}
    catch(secondaryError){console.error('admin-calendar-data:services-fallback',secondaryError);throw secondaryError;}
  }
}
async function activeProviderRates(){
  try{return await lib.sbJson('/rest/v1/provider_service_rates?select=id,provider_id,service_id,rate_name,description,billing_unit,customer_rate,provider_compensation_method,provider_compensation,active,sort_order&active=eq.true&order=provider_id.asc,sort_order.asc,rate_name.asc');}
    ...
    console.warn('admin-calendar-data:rates-primary',primaryError?.message||primaryError);
    return lib.sbJson('/rest/v1/provider_service_rates?select=id,provider_id,service_id,rate_name,description,billing_unit,customer_rate,provider_compensation_method,provider_compensation,active&active=eq.true&order=provider_id.asc,rate_name.asc');
  }
}
async function optional(label,work,warnings){
  try{return await work();}
  catch(e){console.warn(`admin-calendar-data:${label}`,e?.message||e);warnings.push(label);return [];}
}
exports.handler=async event=>{
  ...
  try{
    ...
    // STEP 15.8.6: the core catalog has compatibility fallbacks. Operational side-data
    // is isolated so one optional query can no longer turn the entire Master Calendar
    // into a generic 502 response.
    const [providers,providerServices,services,providerRates]=await Promise.all([
    ...
    let [availability,exceptions,assignments,needsAssignment,scheduleChanges]=await Promise.all([
    ...
    // Pending schedule requests must remain actionable even if the original assignment is outside the displayed week.
    ...
    if(missing.length){
      ...
      const extra=await optional('schedule-change-assignments',()=>lib.sbJson(`/rest/v1/job_assignments?select=${ASSIGN_SELECT}&id=in.(${ids})`),warnings);
    ...
    if(jobIds.length){const list=jobIds.map(x=>encodeURIComponent(x)).join(',');billing=await optional('job-billing',()=>lib.sbJson(`/rest/v1/job_billing_items?select=id,job_id,assignment_id,provider_id,provider_service_rate_id,service_id,service_name,description,quantity,unit,customer_unit_rate,customer_line_total,provider_unit_rate,provider_line_total,gross_profit,unit_rate,line_total,sort_order&job_id=in.(${list})&order=sort_order.asc,id.asc`),warnings);}
    return lib.json(200,{from,to,providers:providers||[],provider_services:providerServices||[],services:services||[],availability:availability||[],exceptions:exceptions||[],assignments:assignments||[],needs_assignment:needsAssignment||[],provider_rates:providerRates||[],job_billing_items:billing||[],schedule_changes:scheduleChanges||[],warnings});
  }catch(e){
    console.error('admin-calendar-data',e);
    const status=e.status||500;
    const message=status===401?'Unauthorized':`Unable to load the Master Calendar${e.message?`: ${e.message}`:'.'}`;
    return lib.json(status,{error:message});
  }
};
