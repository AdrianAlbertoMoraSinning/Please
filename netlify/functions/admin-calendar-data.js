...
exports.handler=async event=>{
  ...
  try{
    ...
    const [providers,providerServices,services,availability,exceptions,assignments,needsAssignment,providerRates,scheduleChanges]=await Promise.all([
      ...
      lib.sbJson('/rest/v1/provider_service_rates?select=id,provider_id,service_id,rate_name,description,billing_unit,customer_rate,provider_compensation,active,sort_order&active=eq.true&order=provider_id.asc,sort_order.asc,rate_name.asc'),
    ...
    if(jobIds.length){const list=jobIds.map(x=>encodeURIComponent(x)).join(',');billing=await lib.sbJson(`/rest/v1/job_billing_items?select=id,job_id,provider_service_rate_id,service_id,service_name,description,quantity,unit,unit_rate,line_total,sort_order&job_id=in.(${list})&order=sort_order.asc,id.asc`);}
