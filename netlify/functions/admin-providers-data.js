...
exports.handler=async function(event){
  ...
  try{
    ...
    if(!id){
      ...
      const enriched=[];for(const p of rows||[]){enriched.push({...p,profile_photo_url:p.profile_image_path?`/.netlify/functions/provider-profile-photo?provider_id=${encodeURIComponent(p.id)}&v=${encodeURIComponent(p.updated_at||Date.now())}`:safePublicUrl(p.profile_image_url),account:byProvider.get(p.id)||null});}return lib.json(200,{providers:enriched});
    }
