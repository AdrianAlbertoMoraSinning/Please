...
exports.handler=async event=>{
  ...
  try{
    ...
    const [providers,ps,services,av,ex,assignRaw,rates,changeRequests,documents,technicalHistory,account]=await Promise.all([
      ...
      lib.sbJson(`/rest/v1/provider_documents?select=id,document_type,document_name,mime_type,file_size_bytes,verification_status,expires_on,review_note,created_at,updated_at&provider_id=eq.${q}&active=eq.true&order=created_at.desc`),
      lib.sbJson(`/rest/v1/provider_technical_history?select=id,event_type,event_label,details,actor_type,created_at&provider_id=eq.${q}&order=created_at.desc&limit=100`),
      lib.sbJson(`/rest/v1/provider_portal_users?select=id,email,display_name,active,last_login_at,password_changed_at,created_at,updated_at&provider_id=eq.${q}&limit=1`).then(x=>x?.[0]||null)
    ]);
