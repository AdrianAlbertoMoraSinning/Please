...
async function optional(label, fn, fallback){
  try{return await fn();}
  catch(e){console.error(`developer-providers:${label}`,e);return fallback;}
}
...
async function listProviders(){
  ...
  try{
    ...
    if(/worker_type|column .* does not exist|schema cache/i.test(String(e.message||''))){
      console.warn('developer-providers: worker_type unavailable; using compatibility mode');
...
async function getProvider(id){
  ...
  try{
    ...
    if(/worker_type|column .* does not exist|schema cache/i.test(String(e.message||''))){
      console.warn('developer-providers: detail worker_type unavailable; using compatibility mode');
...
exports.handler=async function(event){
  ...
  try{
    ...
    console.error('developer-providers',e);
