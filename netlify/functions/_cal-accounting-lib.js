...
async function buildPosting(row){
  ...
  if(['INVOICE_ISSUED','INVOICE_VOIDED','PAYMENT_RECEIVED','STRIPE_FEE_RECORDED','PROVIDER_PAYABLE_CREATED','PROVIDER_PAYMENT_PAID'].includes(type)&&!rule){
...
async function runWorker({limit=25,workerId=`netlify-${crypto.randomBytes(4).toString('hex')}`}={}){
  ...
  const priority={INVOICE_ISSUED:10,PROVIDER_PAYABLE_CREATED:10,PAYMENT_RECEIVED:20,INVOICE_VOIDED:30,PROVIDER_PAYMENT_PAID:30,STRIPE_FEE_RECORDED:40};
...
async function reconcile(limit=200){
  ...
  const summary={queued:0,invoices:0,payments:0,provider_payments:0,errors:[]};
