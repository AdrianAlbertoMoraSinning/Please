...
exports.handler = async function(event) {
  ...
  try {
    ...
    await lib.sbFetch('/rest/v1/admin_portal_login_audit',{method:'POST',headers:{Prefer:'return=minimal'},body:JSON.stringify({email,success,ip_address:lib.requestIp(event)||null,user_agent:lib.requestUserAgent(event)||null})}).catch(()=>{});
    if(!success) return lib.json(401,{error:'Invalid developer email or password.'});
    const session=await lib.createSession(user,event);
    return lib.json(200,{ok:true,user:{email:user.email,display_name:user.display_name,role:user.role}},{'Set-Cookie':lib.sessionCookie(session.token)});
  } catch(error){console.error('developer-login',error);return lib.json(500,{error:'Unable to sign in right now.'});}
};
