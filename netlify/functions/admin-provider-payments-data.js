...
exports.handler=async event=>{
  ...
  try{
    ...
    const [payments,items,providers,jobs,advances]=await Promise.all([
      ...
      lib.sbJson('/rest/v1/provider_advances?select=id,provider_id,job_id,amount,applied_amount,method,reference,note,status,paid_at,created_at&order=created_at.desc')
    ]);
