...
exports.handler=async event=>{
  ...
  try{
    ...
    const [jobs,assignments,providers,services,jobHistory,assignmentHistory,billingItems,scheduleChanges,evidence,serviceRequests,invoices,invoiceItems]=await Promise.all([
      ...
      lib.sbJson('/rest/v1/service_requests?select=id,reference,job_id,status&job_id=not.is.null&order=created_at.desc').catch(()=>[]),
      lib.sbJson('/rest/v1/invoices?select=id,invoice_number,job_id,status,payment_status,subtotal,gst_amount,total_amount,created_at&job_id=not.is.null&status=neq.VOID&order=created_at.desc').catch(()=>[]),
      lib.sbJson('/rest/v1/invoice_items?select=id,invoice_id,description,qty,unit,unit_rate,line_total,sort_order&order=invoice_id.asc,sort_order.asc').catch(()=>[])
    ]);
    for(const x of evidence||[])x.url=await signed(x.storage_path);
    return lib.json(200,{jobs:jobs||[],assignments:assignments||[],providers:providers||[],services:services||[],job_history:jobHistory||[],assignment_history:assignmentHistory||[],job_billing_items:billingItems||[],schedule_changes:scheduleChanges||[],evidence:evidence||[],service_requests:serviceRequests||[],final_invoices:invoices||[],invoice_items:invoiceItems||[]});
