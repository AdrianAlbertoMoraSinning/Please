...
async function serviceChoice(id,currentId=null){if(!validId(id))throw Object.assign(new Error('Select a valid Service Type.'),{status:400});const rows=await lib.sbJson(`/rest/v1/services?select=id,name,active&id=eq.${encodeURIComponent(id)}&limit=1`);const x=rows?.[0];if(!x)throw Object.assign(new Error('Selected Service Type no longer exists.'),{status:400});if(x.active===false&&String(x.id)!==String(currentId||''))throw Object.assign(new Error('Selected Service Type is inactive. Choose an active service.'),{status:409});return x;}
...
exports.handler=async event=>{
 try{
  ...
  if(event.httpMethod==='GET'){
   ...
   const [requests,jobs,assignments,billing,services]=await Promise.all([
    lib.sbJson('/rest/v1/service_requests?select=id,reference,status,first_name,last_name,email,phone,service_id,service_name,street_address,city,province,postal_code,preferred_date,preferred_start_time,estimated_hours,work_description,internal_notes,job_id,created_at,updated_at&order=created_at.desc&limit=500').catch(()=>[]),
    lib.sbJson('/rest/v1/jobs?select=id,reference,status,service_id,service_name,work_address,work_description,estimated_duration_minutes,internal_notes,source_service_request_id,quoted_subtotal,created_at,updated_at,customers(first_name,last_name,email,phone)&order=created_at.desc&limit=500').catch(()=>[]),
    ...
    lib.sbJson('/rest/v1/services?select=id,name,active,sort_order&order=sort_order.asc,name.asc').catch(()=>[])
   ]);
   ...
   rows.sort((a,b)=>new Date(b.updated_at||b.created_at)-new Date(a.updated_at||a.created_at));return lib.json(200,{records:rows.slice(0,500),services:services||[]});
  }
  ...
  if(type==='SERVICE_REQUEST'){
   ...
   const svc=await serviceChoice(patch.service_id,before.service_id),out=requestPatchFrom(patch);out.service_id=svc.id;out.service_name=svc.name;
   const rollback={service_id:before.service_id,service_name:before.service_name,street_address:before.street_address,city:before.city,province:before.province,postal_code:before.postal_code,work_description:before.work_description,preferred_date:before.preferred_date,preferred_start_time:before.preferred_start_time,estimated_hours:before.estimated_hours,internal_notes:before.internal_notes,updated_at:before.updated_at};
   ...
   let rows,jobBefore=null;
   try{
    if(before.job_id)jobBefore=(await lib.sbJson(`/rest/v1/jobs?select=id,service_id,service_name&id=eq.${encodeURIComponent(before.job_id)}&limit=1`))?.[0]||null;
    ...
    if(before.job_id){
      await lib.sbJson(`/rest/v1/jobs?id=eq.${encodeURIComponent(before.job_id)}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({service_id:svc.id,service_name:svc.name,updated_at:new Date().toISOString()})});
   ...
   }catch(e){try{await lib.sbJson(`/rest/v1/service_requests?id=eq.${encodeURIComponent(id)}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify(rollback)});}catch(re){console.error('admin-service-maintenance:request-rollback',re);}if(jobBefore){try{await lib.sbJson(`/rest/v1/jobs?id=eq.${encodeURIComponent(jobBefore.id)}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({service_id:jobBefore.service_id,service_name:jobBefore.service_name,updated_at:new Date().toISOString()})});}catch(re){console.error('admin-service-maintenance:job-classification-rollback',re);}}throw e;}
   await audit(auth,type,id,before.reference,before,before.job_id?'Service Request classification/details and related Job edited in Service Maintenance':'Service Request classification/details edited in Service Maintenance');return lib.json(200,{ok:true,record:rows?.[0],related_job_synchronized:!!before.job_id,service_classification_updated:before.service_id!==svc.id});
  }
  ...
  let sourceBefore=null;if(before.source_service_request_id)sourceBefore=(await lib.sbJson(`/rest/v1/service_requests?select=id,service_id,service_name&id=eq.${encodeURIComponent(before.source_service_request_id)}&limit=1`))?.[0]||null;
  let result;try{
   await lib.sbJson(`/rest/v1/jobs?id=eq.${encodeURIComponent(id)}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({service_id:svc.id,service_name:svc.name,updated_at:new Date().toISOString()})});
   if(sourceBefore)await lib.sbJson(`/rest/v1/service_requests?id=eq.${encodeURIComponent(sourceBefore.id)}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({service_id:svc.id,service_name:svc.name,updated_at:new Date().toISOString()})});
   result=await schedule.updateActiveJob({actorId:auth.user.id,jobId:id,applyToTeam:true,scheduledStart:start,scheduledEnd:end,estimatedDurationMinutes:mins,syncHourlyBilling:patch.sync_hourly_billing!==false,billingOverrides:overrides,syncSourceRequest:true,workAddress:clean(patch.work_address,500),workDescription:clean(patch.work_description,5000),internalNotes:clean(patch.internal_notes,5000),reason:'Job schedule, classification and billing edited in Service Maintenance',notifyPeople:true});
  }catch(e){
   await lib.sbJson(`/rest/v1/jobs?id=eq.${encodeURIComponent(id)}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({service_id:before.service_id,service_name:before.service_name,updated_at:new Date().toISOString()})}).catch(re=>console.error('admin-service-maintenance:job-classification-rollback',re));
   if(sourceBefore)await lib.sbJson(`/rest/v1/service_requests?id=eq.${encodeURIComponent(sourceBefore.id)}`,{method:'PATCH',headers:{Prefer:'return=minimal'},body:JSON.stringify({service_id:sourceBefore.service_id,service_name:sourceBefore.service_name,updated_at:new Date().toISOString()})}).catch(re=>console.error('admin-service-maintenance:source-request-classification-rollback',re));
   throw e;
  }
  const source_request_classification_synced=!before.source_service_request_id||!!sourceBefore;
  await audit(auth,type,id,before.reference,before,'Job schedule / duration / service classification edited in Service Maintenance');return lib.json(200,{ok:true,...result,service_classification_updated:before.service_id!==svc.id,source_request_classification_synced});
