...
exports.handler=async event=>{
  ...
  try{
    ...
    const [invoices,items,jobs,transactions,history]=await Promise.all([
      ...
      lib.sbJson('/rest/v1/jobs?select=id,reference,customer_id,service_id,service_name,work_address,work_description,estimated_duration_minutes,billing_type,customer_rate,billable_quantity,billing_unit,quoted_subtotal,status,created_at,customers(id,first_name,last_name,email,phone)&order=created_at.desc'),
