...
ok(maint.includes('schedule.updateActiveJob')&&maintJs.includes('SAVE JOB + CONFIRM NEW TOTALS')&&maintJs.includes('End Time')&&(/v=(?:15\.9\.2|19\.0)/.test(maintHtml)),'Service Maintenance can edit active Job date/start/end/hours and confirm synchronized billing');
ok(/js\/admin-calendar\.js\?v=15\.(?:9\.1|19\.[2-9]\d*)/.test(calHtml)&&/js\/admin-service-requests\.js\?v=15\.(?:9\.1|19\.[2-9]\d*)/.test(reqHtml),'Admin schedule screens are cache-busted for STEP 15.9.1 or newer');
