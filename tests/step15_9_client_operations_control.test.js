...
const providerHtml=read('provider.html'),providerJs=read('js/provider.js'),liveFn=read('netlify/functions/provider-live-service-action.js'),evidenceFn=read('netlify/functions/provider-service-evidence-upload.js'),evidenceLib=read('netlify/functions/_provider-evidence-lib.js'),trackingFn=read('netlify/functions/public-request-tracking.js');
...
ok(evidenceLib.includes("new Set(['CHECK_IN','ARRIVAL','COMPLETION','CHECK_OUT'])")&&liveFn.includes("CHECK_IN:'CHECK_IN'")&&liveFn.includes("CHECK_OUT:'CHECK_OUT'")&&evidenceLib.includes("TZ='America/Edmonton'"),'PLEASE Staff retains all four evidence types while STEP 19 applies Check In/Out at the daily workday boundary');
...
ok(sw.includes("please-provider-v19"),'Provider PWA cache advances for the STEP 19 daily workflow');
