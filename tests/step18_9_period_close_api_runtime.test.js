...
const fake={
 ...
 sbJson:async(url,opt={})=>{const body=opt.body?JSON.parse(opt.body):null;calls.push({url,method:opt.method||'GET',body});
   if(url==='/rest/v1/rpc/accounting_prepare_period_close')return[{accounting_prepare_period_close:'period-1'}];
...
(async()=>{
 ...
 r=await fn.handler({httpMethod:'POST',headers:{},body:JSON.stringify({action:'PREPARE',payload:{period_start:'2026-08-01',period_end:'2026-08-31'}})});j=JSON.parse(r.body);assert.strictEqual(r.statusCode,200);assert.strictEqual(j.result.period_id,'period-1');assert.strictEqual(j.data.selectedPeriod.id,'period-1');assert.strictEqual(j.data.summary.reviews,1);const rpc=calls.find(c=>c.url==='/rest/v1/rpc/accounting_prepare_period_close');assert.deepStrictEqual(rpc.body,{p_period_start:'2026-08-01',p_period_end:'2026-08-31',p_actor_id:'admin-1'});
